...
const parentTree = require("../delijnParentTree.json");
  ...
  async.forEachOf(this._entrypoints, function (entryUrl, i, done) {
    ...
    if (i === 0) {
      ...
      connectionsStream = new MergeSortStream(connectionsStream, newConnectionsStream, function (connectionA, connectionB) {
        connectionA["arrivalStop"] = findParent(connectionA["arrivalStop"]);
        connectionA["departureStop"] = findParent(connectionA["departureStop"]);
        connectionB["arrivalStop"] = findParent(connectionB["arrivalStop"]);
        connectionB["departureStop"] = findParent(connectionB["departureStop"]);
